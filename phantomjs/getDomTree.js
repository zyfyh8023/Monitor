module.exports = function(TOKEN, data) {
     ...
     * @returns {boolean}
    */
    var IGNORE_CHILDREN_ELEMENT=[
        'img', 'canvas', 'input', 'textarea', 'audio','video',
        'hr', 'embed', 'object', 'progress', 'select', 'table'
    ];
    ...
    var IGNORE_CHILDREN_ID=['wrapperimg', 'voucher'];
    function ignoreChildrenId(elem){
        var idName=elem.getAttribute('id');
        if(idName && IGNORE_CHILDREN_ID.length){
            for(var i=0; i<IGNORE_CHILDREN_ID.length; i++){
                if(idName==IGNORE_CHILDREN_ID[i]){
                    return true;
                }
            }
        }
    ...
    //暂未使用
    var IGNORE_CHILDREN_CLASS=[];
    function ignoreChildrenClass(elem){
        var className=elem.getAttribute('class');
        if(className && IGNORE_CHILDREN_CLASS.length){
            for(var i=0; i<IGNORE_CHILDREN_CLASS.length; i++){
                if(className==IGNORE_CHILDREN_CLASS[i]){
                    return true;
                }
            }
        }
     ...
     * @returns {string}
     */
    var STYLE_FILTERS=['font-size', 'display', 'opacity', 'visibility', 'text-align',
    ...
    function getRect(elem) {
        ...
        var doc = elem.ownerDocument;  //document html
    ...
    function mapOtherAttr(elem) {
        var ret = {};
        var hasOtherAttr = false;
        for (var key in elem.attributes) {
            if (elem.attributes[key].name && elem.attributes[key].value) {
                var attrName = elem.attributes[key].name.toLowerCase();
                if (attrName != 'class' && attrName != 'id' && attrName != 'style') {
                    ret[attrName] = elem.attributes[key].value;
                    hasOtherAttr = true;
                }
            }
        }
        return hasOtherAttr ? ret : false;
    }

    function getInnerText(elem){
        var innerText=typeof elem.textContent =='string' ? elem.textContent :elem.innerText;
        if(innerText) innerText=innerText.trim();
        return innerText;
    }
    /**
     * walk dom tree
    ...
    function walk(elem) {
        ...
        if (elem.nodeType === 1) { // element
            ...
            node.otherAttr = mapOtherAttr(elem);
            node.style = getStyles(elem);
            node.text=md5(getInnerText(elem));
            node.child = [];
            if (igonreChildren(elem) || ignoreChildrenId(elem)) {
                node.child.push({
                    name: '#',
    ...
    var REMOVE_SELECTORS = ['script', 'style'];

    if (REMOVE_SELECTORS && REMOVE_SELECTORS.length) {
    ...
    //弹窗运营推广
    var REMOVE_CLASS=['.J_mask_container', '.activity-mid', '.mod-dialog-frame'];

    if (REMOVE_CLASS && REMOVE_CLASS.length) {
        REMOVE_CLASS.forEach(function(selector) {
            var elems = document.querySelectorAll(selector);
            for (var i = 0, len = elems.length; i < len; i++) {
                var elem = elems[i];
                elem.parentNode.removeChild(elem);
    ...
    return walk(document.querySelector('body'));
